    ...
    IEquatable<Variant<T1, T2, T3, T4, T5, T6, T7, T8>>
{
        ...
        Action<T5>? action5, Action<T6>? action6, Action<T7>? action7, Action<T8>? action8)
    {
        ...
        if (_setItem == SetItems.Item8 && _value is VariantValue<T8> {Item : { } value8}) action8?.Invoke(value8);
    }
        ...
        Func<T8, TResult>? func8)
    {
        return _setItem switch
        {
            ...
            SetItems.Item8 when _value is VariantValue<T8> {Item : { } value8} && func8 is not null => func8(value8),
            _ => default
        };
    }
